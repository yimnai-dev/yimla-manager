...
function convertAwaitBlock(node, parent, ctx) {
    ...
    if (!node.catch.skip) {
        ...
        const catchStart = awaitBlock.pending || awaitBlock.then ? node.catch.start : nodeStart;
...
function convertSnippetBlock(node, parent, ctx) {
    ...
    ctx.scriptLet.nestSnippetBlock(node.expression, closeParenIndex, snippetBlock, (id, context) => {
        snippetBlock.id = id;
        snippetBlock.context = context;
    });
    snippetBlock.children.push(...(0, element_1.convertChildren)(node, snippetBlock, ctx));
    ctx.scriptLet.closeScope();
